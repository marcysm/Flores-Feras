...
function cacheDefinitiveElements() {
  const ids = [
    ...
    "map-location-icon-url", "map-location-enabled", "map-location-secret",
...
function renderMapCanvas() {
  ...
  elements.mapEditorCanvas.querySelectorAll(".map-editor-node").forEach(node => node.remove());
  elements.mapEditorEmpty?.classList.toggle("is-hidden", state.mapLocations.length > 0);
  state.mapLocations.forEach(location => {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "map-editor-node";
    button.textContent = location.name || location.location_key;
    button.style.left = `${Math.max(0, Math.min(100, Number(location.map_x ?? 50)))}%`;
    button.style.top = `${Math.max(0, Math.min(100, Number(location.map_y ?? 50)))}%`;
    button.dataset.locationId = location.id;
    button.addEventListener("click", () => openEditMapLocationModal(location.id));
...
async function saveMapLocation(event) {
  ...
  try {
    ...
    await callAdminRpc("admin_save_map_location", { p_data: payload });
...
async function saveMapConnection(event) {
  ...
  try {
    ...
    await callAdminRpc("admin_save_map_connection", { p_data: payload });
    await loadMapData(true);
