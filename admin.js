  ...
  try {
    switch (section) {
      ...
      case "map":
        ...
        break;
      case "scenarios":
        await loadScenarioAdminData();
        break;
      case "characters":
        await loadCharacterAdminData();
        break;
      case "progress":
...
/* ==========================================================
   EXTENSÃO ADMIN — CENÁRIOS E PERSONAGENS
   ========================================================== */
...
function initializeStoryExtensionAdmin() {
  [
    "scenario-search", "scenario-admin-message",
    "scenario-admin-list", "scenario-admin-modal",
    "scenario-admin-form", "scenario-admin-id",
    "scenario-admin-scene-id", "scenario-admin-title",
    "scenario-admin-name", "scenario-admin-description",
    "scenario-admin-ascii", "scenario-admin-image",
    "scenario-admin-alt", "scenario-admin-unavailable",
    "scenario-admin-allow", "scenario-admin-enabled",
    "scenario-admin-form-message", "new-scenario-detail-button",
    "scenario-detail-list", "scenario-detail-modal",
    "scenario-detail-form", "scenario-detail-id",
    "scenario-detail-title", "scenario-detail-description",
    "scenario-detail-conditions", "scenario-detail-order",
    "scenario-detail-enabled", "scenario-detail-message",

    "new-character-button", "character-search",
    "character-admin-message", "character-admin-list",
    "character-modal", "character-form", "character-id",
    "character-name", "character-key", "character-description",
    "character-portrait", "character-side", "character-ascii",
    "character-color", "character-commands",
    "character-enabled", "character-form-message",

    "appearance-modal", "appearance-character-title",
    "new-appearance-button", "appearance-list",
    "dialogue-editor", "dialogue-appearance-title",
    "new-dialogue-node-button", "dialogue-node-list",

    "appearance-form-modal", "appearance-form",
    "appearance-id", "appearance-scene", "appearance-intro",
    "appearance-outro", "appearance-commands",
    "appearance-conditions", "appearance-enabled",
    "appearance-form-message",

    "dialogue-node-modal", "dialogue-node-form",
    "dialogue-node-id", "dialogue-node-key",
    "dialogue-node-text", "dialogue-node-fallback",
    "dialogue-node-start", "dialogue-node-ending",
    "dialogue-node-enabled", "dialogue-node-message",
    "dialogue-response-editor",
    "new-dialogue-response-button",
    "dialogue-response-list",

    "dialogue-response-modal", "dialogue-response-form",
    "dialogue-response-id", "dialogue-response-name",
    "dialogue-response-mode", "dialogue-response-exact",
    "dialogue-response-required", "dialogue-response-optional",
    "dialogue-response-forbidden", "dialogue-response-synonyms",
    "dialogue-response-character", "dialogue-response-next",
    "dialogue-response-priority", "dialogue-response-conditions",
    "dialogue-response-effects", "dialogue-response-end",
    "dialogue-response-enabled", "dialogue-response-message"
  ].forEach(id => {
    extensionElements[toCamelCase(id)] =
      document.getElementById(id);
  });

  document.querySelectorAll("[data-extension-close]")
    .forEach(button => {
      button.addEventListener("click", () => {
        closeStoryExtensionModals();
      });
    });

  extensionElements.scenarioSearch?.addEventListener(
    "input",
    renderScenarioAdminList
  );

  extensionElements.scenarioAdminList?.addEventListener(
    "click",
    handleScenarioAdminListClick
  );

  extensionElements.scenarioAdminForm?.addEventListener(
    "submit",
    saveScenarioAdmin
  );

  extensionElements.newScenarioDetailButton?.addEventListener(
    "click",
    openNewScenarioDetail
  );

  extensionElements.scenarioDetailList?.addEventListener(
    "click",
    handleScenarioDetailClick
  );

  extensionElements.scenarioDetailForm?.addEventListener(
    "submit",
    saveScenarioDetail
  );

  extensionElements.newCharacterButton?.addEventListener(
    "click",
    openNewCharacter
  );

  extensionElements.characterSearch?.addEventListener(
    "input",
    renderCharacterAdminList
  );

  extensionElements.characterAdminList?.addEventListener(
    "click",
    handleCharacterAdminListClick
  );

  extensionElements.characterForm?.addEventListener(
    "submit",
    saveCharacter
  );

  extensionElements.newAppearanceButton?.addEventListener(
    "click",
    openNewAppearance
  );

  extensionElements.appearanceList?.addEventListener(
    "click",
    handleAppearanceListClick
  );

  extensionElements.appearanceForm?.addEventListener(
    "submit",
    saveAppearance
  );

  extensionElements.newDialogueNodeButton?.addEventListener(
    "click",
    openNewDialogueNode
  );

  extensionElements.dialogueNodeList?.addEventListener(
    "click",
    handleDialogueNodeListClick
  );

  extensionElements.dialogueNodeForm?.addEventListener(
    "submit",
    saveDialogueNode
  );

  extensionElements.newDialogueResponseButton?.addEventListener(
    "click",
    openNewDialogueResponse
  );

  extensionElements.dialogueResponseList?.addEventListener(
    "click",
    handleDialogueResponseListClick
  );

  extensionElements.dialogueResponseForm?.addEventListener(
    "submit",
    saveDialogueResponse
  );

  populateExtensionSceneSelector();
}
...
function extensionOpen(modal) {
  modal?.classList.remove("is-hidden");
  document.body.style.overflow = "hidden";
}

function closeStoryExtensionModals() {
  [
    extensionElements.scenarioAdminModal,
    extensionElements.scenarioDetailModal,
    extensionElements.characterModal,
    extensionElements.appearanceModal,
    extensionElements.appearanceFormModal,
    extensionElements.dialogueNodeModal,
    extensionElements.dialogueResponseModal
...
function extensionMessage(element, text = "", error = false) {
  if (!element) return;
  element.textContent = text;
  element.classList.toggle("is-error", error);
  element.classList.toggle("is-success", !error && Boolean(text));
}
...
function populateExtensionSceneSelector() {
  const select = extensionElements.appearanceScene;
  if (!select) return;
  ...
  [...state.scenes]
    .sort((a, b) =>
      String(a.title || a.scene_key).localeCompare(
        String(b.title || b.scene_key),
        "pt-BR"
      )
    )
    .forEach(scene => {
      ...
      option.value = scene.id;
      option.textContent =
        `${scene.title || scene.scene_key} — ${scene.scene_key}`;
      select.appendChild(option);
    });
}
...
async function loadScenarioAdminData() {
  const { data, error } = await state.client
    .from("scene_scenarios")
    .select("*")
    .eq("game_id", state.game.id);

  if (error) throw error;

  extensionState.scenarios = data || [];
  renderScenarioAdminList();
}

function renderScenarioAdminList() {
  const list = extensionElements.scenarioAdminList;
  if (!list) return;

  list.replaceChildren();

  const search = String(
    extensionElements.scenarioSearch?.value || ""
  ).toLocaleLowerCase("pt-BR");

  const scenarioByScene = new Map(
    extensionState.scenarios.map(item => [
      item.scene_id,
      item
    ])
  );
  ...
  const scenes = state.scenes.filter(scene => {
    const haystack =
      `${scene.title} ${scene.scene_key}`.toLocaleLowerCase("pt-BR");

    return !search || haystack.includes(search);
  });

  extensionMessage(
    extensionElements.scenarioAdminMessage,
    `${scenes.length} Cenas disponíveis.`
  );

  scenes.forEach(scene => {
    const scenario = scenarioByScene.get(scene.id);

    const card = document.createElement("article");
    card.className = "scene-card extension-card";

    card.innerHTML = `
      <div>
        <p class="extension-card__eyebrow">
          ${scenario ? "CONFIGURADO" : "SEM CENÁRIO"}
        </p>
        <h3>${escapeExtensionHtml(
          scene.title || scene.scene_key
        )}</h3>
        <p>${escapeExtensionHtml(
          scenario?.description ||
          "Nenhuma descrição visual cadastrada."
        )}</p>
      </div>
      <button class="button button--secondary"
        type="button"
        data-edit-scenario="${scene.id}">
        EDITAR CENÁRIO
      </button>
    `;
...
function handleScenarioAdminListClick(event) {
  const button = event.target.closest("[data-edit-scenario]");
  if (!button) return;

  openScenarioAdmin(button.dataset.editScenario);
}

async function openScenarioAdmin(sceneId) {
  const scene = state.scenes.find(item => item.id === sceneId);
  const scenario = extensionState.scenarios.find(
    item => item.scene_id === sceneId
  );

  extensionElements.scenarioAdminId.value =
    scenario?.id || "";
  extensionElements.scenarioAdminSceneId.value = sceneId;
  extensionElements.scenarioAdminTitle.textContent =
    scene?.title || scene?.scene_key || "Cenário";
  extensionElements.scenarioAdminName.value =
    scenario?.title || "";
  extensionElements.scenarioAdminDescription.value =
    scenario?.description || "";
  extensionElements.scenarioAdminAscii.value =
    scenario?.ascii_art || "";
  extensionElements.scenarioAdminImage.value =
    scenario?.image_url || "";
  extensionElements.scenarioAdminAlt.value =
    scenario?.image_alt_text || "";
  extensionElements.scenarioAdminUnavailable.value =
    scenario?.unavailable_text ||
    "Não há nada especial para observar aqui.";
  extensionElements.scenarioAdminAllow.checked =
    scenario?.allow_scenario !== false;
  extensionElements.scenarioAdminEnabled.checked =
    scenario?.is_enabled !== false;

  extensionState.scenarioDetails = [];

  if (scenario?.id) {
    const { data, error } = await state.client
      .from("scene_scenario_details")
      .select("*")
      .eq("scenario_id", scenario.id)
      .order("display_order");

    if (error) throw error;
    extensionState.scenarioDetails = data || [];
  }

  renderScenarioDetails();
  extensionOpen(extensionElements.scenarioAdminModal);
}

async function saveScenarioAdmin(event) {
  event.preventDefault();

  try {
    const id =
      extensionElements.scenarioAdminId.value || undefined;

    const payload = {
      id,
      game_id: state.game.id,
      scene_id: extensionElements.scenarioAdminSceneId.value,
      title: extensionElements.scenarioAdminName.value.trim() || null,
      description:
        extensionElements.scenarioAdminDescription.value.trim() || null,
      ascii_art:
        extensionElements.scenarioAdminAscii.value || null,
      image_url:
        extensionElements.scenarioAdminImage.value.trim() || null,
      image_alt_text:
        extensionElements.scenarioAdminAlt.value.trim() || null,
      unavailable_text:
        extensionElements.scenarioAdminUnavailable.value.trim() ||
        "Não há nada especial para observar aqui.",
      allow_scenario:
        extensionElements.scenarioAdminAllow.checked,
      is_enabled:
    ...
    extensionElements.scenarioAdminId.value = data.id;
    await loadScenarioAdminData();

    extensionMessage(
      extensionElements.scenarioAdminFormMessage,
      "Cenário salvo."
    );
  } catch (error) {
    extensionMessage(
...
function renderScenarioDetails() {
  const list = extensionElements.scenarioDetailList;
  list.replaceChildren();

  extensionState.scenarioDetails.forEach(detail => {
    const card = document.createElement("article");
    card.className = "extension-mini-card";
    card.innerHTML = `
      <div>
        <strong>${escapeExtensionHtml(
          detail.title || "Detalhe sem título"
        )}</strong>
        <p>${escapeExtensionHtml(detail.description)}</p>
      </div>
      <div class="extension-card__actions">
        <button type="button"
          data-edit-scenario-detail="${detail.id}">EDITAR</button>
        <button type="button"
          data-delete-scenario-detail="${detail.id}">APAGAR</button>
      </div>
    `;
    list.appendChild(card);
  });
...
function openNewScenarioDetail() {
  if (!extensionElements.scenarioAdminId.value) {
    extensionMessage(
      extensionElements.scenarioAdminFormMessage,
      "Salve primeiro o Cenário principal.",
      true
    );
    return;
  }

  extensionElements.scenarioDetailForm.reset();
  extensionElements.scenarioDetailId.value = "";
  extensionElements.scenarioDetailConditions.value = "[]";
  extensionElements.scenarioDetailOrder.value = "0";
  extensionElements.scenarioDetailEnabled.checked = true;
  extensionOpen(extensionElements.scenarioDetailModal);
}

async function handleScenarioDetailClick(event) {
  const edit = event.target.closest("[data-edit-scenario-detail]");
  const remove = event.target.closest("[data-delete-scenario-detail]");

  if (edit) {
    const detail = extensionState.scenarioDetails.find(
      item => item.id === edit.dataset.editScenarioDetail
    );
    if (!detail) return;

    extensionElements.scenarioDetailId.value = detail.id;
    extensionElements.scenarioDetailTitle.value = detail.title || "";
    extensionElements.scenarioDetailDescription.value =
      detail.description || "";
    extensionElements.scenarioDetailConditions.value =
      JSON.stringify(detail.condition_rules || [], null, 2);
    extensionElements.scenarioDetailOrder.value =
      detail.display_order || 0;
    extensionElements.scenarioDetailEnabled.checked =
      detail.is_enabled !== false;

    extensionOpen(extensionElements.scenarioDetailModal);
  }

  if (remove && confirm("Apagar este detalhe?")) {
    const { error } = await state.client
      .from("scene_scenario_details")
      .delete()
      .eq("id", remove.dataset.deleteScenarioDetail);

    if (error) throw error;

    await openScenarioAdmin(
      extensionElements.scenarioAdminSceneId.value
    );
  }
}

async function saveScenarioDetail(event) {
  event.preventDefault();

  try {
    const id =
      extensionElements.scenarioDetailId.value || undefined;

    const payload = {
      id,
      scenario_id: extensionElements.scenarioAdminId.value,
      title:
        extensionElements.scenarioDetailTitle.value.trim() || null,
      description:
        extensionElements.scenarioDetailDescription.value.trim(),
      condition_rules: jsonValue(
        extensionElements.scenarioDetailConditions.value,
        []
      ),
      display_order:
        Number(extensionElements.scenarioDetailOrder.value) || 0,
      is_enabled:
        extensionElements.scenarioDetailEnabled.checked,
      updated_at: new Date().toISOString()
    };
    ...
    extensionElements.scenarioDetailModal.classList.add("is-hidden");
    ...
    extensionMessage(
      extensionElements.scenarioDetailMessage,
      formatDatabaseError(error),
      true
    );
  }
...
async function loadCharacterAdminData() {
  const { data, error } = await state.client
    .from("characters")
    .select("*")
    .eq("game_id", state.game.id)
  ...
  extensionState.characters = data || [];
  renderCharacterAdminList();
}

function renderCharacterAdminList() {
  const list = extensionElements.characterAdminList;
  list.replaceChildren();

  const search = String(
    extensionElements.characterSearch?.value || ""
  ).toLocaleLowerCase("pt-BR");

  const characters = extensionState.characters.filter(character => {
    const haystack =
      `${character.name} ${character.character_key}`
        .toLocaleLowerCase("pt-BR");
  ...
  extensionMessage(
    extensionElements.characterAdminMessage,
    `${characters.length} Personagens cadastradas.`
  );
  ...
  characters.forEach(character => {
    ...
    card.innerHTML = `
      <div>
        <p class="extension-card__eyebrow">
          ${character.is_enabled ? "ATIVA" : "DESATIVADA"}
        </p>
        <h3>${escapeExtensionHtml(character.name)}</h3>
        <p>${escapeExtensionHtml(
      ...
      <div class="extension-card__actions">
        <button type="button"
          data-character-appearances="${character.id}">
          CENAS E DIÁLOGOS
        </button>
        <button type="button"
...
function openNewCharacter() {
  extensionElements.characterForm.reset();
  extensionElements.characterId.value = "";
  extensionElements.characterSide.value = "left";
  extensionElements.characterEnabled.checked = true;
  extensionOpen(extensionElements.characterModal);
}

async function handleCharacterAdminListClick(event) {
  const edit = event.target.closest("[data-edit-character]");
  const appearances = event.target.closest(
    "[data-character-appearances]"
  );
  const remove = event.target.closest("[data-delete-character]");

  if (edit) {
    const character = extensionState.characters.find(
      item => item.id === edit.dataset.editCharacter
    );
    if (!character) return;

    extensionElements.characterId.value = character.id;
    extensionElements.characterName.value = character.name;
    extensionElements.characterKey.value = character.character_key;
    extensionElements.characterDescription.value =
      character.description || "";
    extensionElements.characterPortrait.value =
      character.portrait_url || "";
    extensionElements.characterSide.value =
      character.portrait_side || "left";
    extensionElements.characterAscii.value =
      character.ascii_art || "";
    extensionElements.characterColor.value =
      character.name_color || "";
    extensionElements.characterCommands.value =
      (character.start_commands || []).join(", ");
    extensionElements.characterEnabled.checked =
  ...
  if (appearances) {
    await openCharacterAppearances(
      appearances.dataset.characterAppearances
    );
  }

  if (remove && confirm(
    "Apagar esta Personagem e todas as conversas dela?"
  )) {
    const { error } = await state.client
      .from("characters")
      .delete()
      .eq("id", remove.dataset.deleteCharacter);
...
async function saveCharacter(event) {
  ...
  try {
    const id = extensionElements.characterId.value || undefined;

    const payload = {
      id,
      game_id: state.game.id,
      character_key:
        extensionElements.characterKey.value.trim(),
      name:
        extensionElements.characterName.value.trim(),
      description:
        extensionElements.characterDescription.value.trim() || null,
      portrait_url:
        extensionElements.characterPortrait.value.trim() || null,
      portrait_side:
        extensionElements.characterSide.value,
      ascii_art:
        extensionElements.characterAscii.value || null,
      name_color:
        extensionElements.characterColor.value.trim() || null,
      start_commands:
        csvArray(extensionElements.characterCommands.value),
      is_enabled:
        extensionElements.characterEnabled.checked,
      updated_at: new Date().toISOString()
    };
    ...
    extensionElements.characterModal.classList.add("is-hidden");
    await loadCharacterAdminData();
  } catch (error) {
    extensionMessage(
      extensionElements.characterFormMessage,
      formatDatabaseError(error),
      true
    );
  }
...
async function openCharacterAppearances(characterId) {
  extensionState.selectedCharacterId = characterId;

  const character = extensionState.characters.find(
    item => item.id === characterId
  );

  extensionElements.appearanceCharacterTitle.textContent =
  ...
  extensionState.appearances = data || [];
  extensionState.selectedAppearanceId = null;

  renderAppearanceList();
  extensionElements.dialogueEditor.classList.add("is-hidden");
  extensionOpen(extensionElements.appearanceModal);
}

function renderAppearanceList() {
  const list = extensionElements.appearanceList;
  list.replaceChildren();

  extensionState.appearances.forEach(appearance => {
    const scene = state.scenes.find(
      item => item.id === appearance.scene_id
    );
    ...
    card.innerHTML = `
      <div>
        <strong>${escapeExtensionHtml(
          scene?.title || scene?.scene_key || "Cena"
        )}</strong>
        <p>${escapeExtensionHtml(
          appearance.intro_text ||
          "Sem texto introdutório."
        )}</p>
      </div>
      <div class="extension-card__actions">
        <button type="button"
          data-open-dialogue="${appearance.id}">
          DIÁLOGO
        </button>
        <button type="button"
          data-edit-appearance="${appearance.id}">
          EDITAR
        </button>
        <button type="button"
          data-delete-appearance="${appearance.id}">
          APAGAR
        </button>
      </div>
    `;
...
function openNewAppearance() {
  extensionElements.appearanceForm.reset();
  extensionElements.appearanceId.value = "";
  extensionElements.appearanceConditions.value = "[]";
  extensionElements.appearanceEnabled.checked = true;
  populateExtensionSceneSelector();
  extensionOpen(extensionElements.appearanceFormModal);
}

async function handleAppearanceListClick(event) {
  const edit = event.target.closest("[data-edit-appearance]");
  const dialogue = event.target.closest("[data-open-dialogue]");
  const remove = event.target.closest("[data-delete-appearance]");

  if (edit) {
    const appearance = extensionState.appearances.find(
      item => item.id === edit.dataset.editAppearance
    );
    if (!appearance) return;

    extensionElements.appearanceId.value = appearance.id;
    extensionElements.appearanceScene.value = appearance.scene_id;
    extensionElements.appearanceIntro.value =
      appearance.intro_text || "";
    extensionElements.appearanceOutro.value =
      appearance.outro_text || "";
    extensionElements.appearanceCommands.value =
      (appearance.start_commands || []).join(", ");
    extensionElements.appearanceConditions.value =
      JSON.stringify(appearance.condition_rules || [], null, 2);
    extensionElements.appearanceEnabled.checked =
  ...
  if (dialogue) {
    await openDialogueEditor(dialogue.dataset.openDialogue);
  }

  if (remove && confirm("Apagar esta Aparição e seu Diálogo?")) {
    const { error } = await state.client
      .from("scene_characters")
      .delete()
      .eq("id", remove.dataset.deleteAppearance);

    if (error) throw error;

    await openCharacterAppearances(
      extensionState.selectedCharacterId
    );
  }
}
...
async function saveAppearance(event) {
  ...
  try {
    const id = extensionElements.appearanceId.value || undefined;

    const payload = {
      id,
      game_id: state.game.id,
      character_id: extensionState.selectedCharacterId,
      scene_id: extensionElements.appearanceScene.value,
      intro_text:
        extensionElements.appearanceIntro.value.trim() || null,
      outro_text:
        extensionElements.appearanceOutro.value.trim() || null,
      start_commands:
        csvArray(extensionElements.appearanceCommands.value),
      condition_rules:
        jsonValue(extensionElements.appearanceConditions.value, []),
      is_enabled:
        extensionElements.appearanceEnabled.checked,
      updated_at: new Date().toISOString()
    };
    ...
    extensionElements.appearanceFormModal.classList.add("is-hidden");
    ...
    extensionMessage(
      extensionElements.appearanceFormMessage,
      formatDatabaseError(error),
      true
    );
  }
...
async function openDialogueEditor(appearanceId) {
  extensionState.selectedAppearanceId = appearanceId;
  ...
  extensionElements.dialogueAppearanceTitle.textContent =
    scene?.title || scene?.scene_key || "Conversa";
  ...
  extensionState.nodes = data || [];
  renderDialogueNodeList();
  extensionElements.dialogueEditor.classList.remove("is-hidden");
}

function renderDialogueNodeList() {
  const list = extensionElements.dialogueNodeList;
  list.replaceChildren();

  extensionState.nodes.forEach(node => {
    const card = document.createElement("article");
    card.className = "extension-mini-card";

    card.innerHTML = `
      <div>
        <strong>${escapeExtensionHtml(node.node_key)}
          ${node.is_start ? " — INICIAL" : ""}
        </strong>
        <p>${escapeExtensionHtml(node.speaker_text)}</p>
      </div>
      <div class="extension-card__actions">
        <button type="button"
          data-edit-dialogue-node="${node.id}">
          EDITAR E RESPOSTAS
        </button>
        <button type="button"
          data-delete-dialogue-node="${node.id}">
          APAGAR
        </button>
      </div>
    `;
...
function openNewDialogueNode() {
  extensionElements.dialogueNodeForm.reset();
  extensionElements.dialogueNodeId.value = "";
  extensionElements.dialogueNodeFallback.value =
    "A personagem não entendeu o que você quis dizer.";
  extensionElements.dialogueNodeEnabled.checked = true;
  extensionElements.dialogueResponseEditor.classList.add("is-hidden");
  extensionOpen(extensionElements.dialogueNodeModal);
}

async function handleDialogueNodeListClick(event) {
  const edit = event.target.closest("[data-edit-dialogue-node]");
  const remove = event.target.closest("[data-delete-dialogue-node]");

  if (edit) {
    const node = extensionState.nodes.find(
      item => item.id === edit.dataset.editDialogueNode
    );
    if (!node) return;

    extensionState.selectedNodeId = node.id;

    extensionElements.dialogueNodeId.value = node.id;
    extensionElements.dialogueNodeKey.value = node.node_key;
    extensionElements.dialogueNodeText.value = node.speaker_text;
    extensionElements.dialogueNodeFallback.value =
      node.fallback_text;
    extensionElements.dialogueNodeStart.checked = node.is_start;
    extensionElements.dialogueNodeEnding.checked = node.is_ending;
    extensionElements.dialogueNodeEnabled.checked =
      node.is_enabled !== false;

    await loadDialogueResponses(node.id);
    extensionElements.dialogueResponseEditor.classList.remove(
      "is-hidden"
    );
    extensionOpen(extensionElements.dialogueNodeModal);
  }
  ...
  if (remove && confirm(
    "Apagar esta Fala e todas as Respostas?"
  )) {
    const { error } = await state.client
      .from("dialogue_nodes")
      .delete()
      .eq("id", remove.dataset.deleteDialogueNode);

    if (error) throw error;

    await openDialogueEditor(
      extensionState.selectedAppearanceId
    );
  }
}

async function saveDialogueNode(event) {
  event.preventDefault();

  try {
    const id =
      extensionElements.dialogueNodeId.value || undefined;

    if (extensionElements.dialogueNodeStart.checked) {
      await state.client
        .from("dialogue_nodes")
    ...
    const payload = {
      id,
      appearance_id: extensionState.selectedAppearanceId,
      node_key:
        extensionElements.dialogueNodeKey.value.trim(),
      speaker_text:
        extensionElements.dialogueNodeText.value.trim(),
      fallback_text:
        extensionElements.dialogueNodeFallback.value.trim(),
      is_start:
        extensionElements.dialogueNodeStart.checked,
      is_ending:
        extensionElements.dialogueNodeEnding.checked,
      is_enabled:
        extensionElements.dialogueNodeEnabled.checked,
      updated_at: new Date().toISOString()
    };
    ...
    extensionState.selectedNodeId = data.id;
    extensionElements.dialogueNodeId.value = data.id;

    await openDialogueEditor(
      extensionState.selectedAppearanceId
    );

    await loadDialogueResponses(data.id);
    extensionElements.dialogueResponseEditor.classList.remove(
      "is-hidden"
    );

    extensionMessage(
      extensionElements.dialogueNodeMessage,
      "Fala salva."
    );
  } catch (error) {
    extensionMessage(
...
async function loadDialogueResponses(nodeId) {
  const { data, error } = await state.client
  ...
  extensionState.responses = data || [];
  renderDialogueResponseList();
  populateDialogueNextNodeSelector();
}

function renderDialogueResponseList() {
  const list = extensionElements.dialogueResponseList;
  list.replaceChildren();

  extensionState.responses.forEach(response => {
    const card = document.createElement("article");
    card.className = "extension-mini-card";
    ...
    card.innerHTML = `
      ...
      <div class="extension-card__actions">
        <button type="button"
          data-edit-dialogue-response="${response.id}">
          EDITAR
        </button>
        <button type="button"
          data-delete-dialogue-response="${response.id}">
          APAGAR
        </button>
      </div>
    `;
...
function populateDialogueNextNodeSelector() {
  const select = extensionElements.dialogueResponseNext;
  select.replaceChildren();

  const empty = document.createElement("option");
  empty.value = "";
  empty.textContent = "Permanecer nesta fala";
  select.appendChild(empty);
  ...
  extensionState.nodes.forEach(node => {
    ...
    option.textContent =
      `${node.node_key}${node.is_start ? " — inicial" : ""}`;
    select.appendChild(option);
  });
}

function openNewDialogueResponse() {
  if (!extensionState.selectedNodeId) return;

  extensionElements.dialogueResponseForm.reset();
  extensionElements.dialogueResponseId.value = "";
  extensionElements.dialogueResponseMode.value = "keywords";
  extensionElements.dialogueResponseSynonyms.value = "{}";
  extensionElements.dialogueResponseConditions.value = "[]";
  extensionElements.dialogueResponseEffects.value = "[]";
  extensionElements.dialogueResponsePriority.value = "100";
  extensionElements.dialogueResponseEnabled.checked = true;

  populateDialogueNextNodeSelector();
  extensionOpen(extensionElements.dialogueResponseModal);
}

async function handleDialogueResponseListClick(event) {
  const edit = event.target.closest(
    "[data-edit-dialogue-response]"
  );
  const remove = event.target.closest(
    "[data-delete-dialogue-response]"
  );

  if (edit) {
    const response = extensionState.responses.find(
      item => item.id === edit.dataset.editDialogueResponse
    );
    if (!response) return;

    extensionElements.dialogueResponseId.value = response.id;
    extensionElements.dialogueResponseName.value =
      response.internal_name;
    extensionElements.dialogueResponseMode.value =
      response.match_mode;
    extensionElements.dialogueResponseExact.value =
      response.exact_phrase || "";
    extensionElements.dialogueResponseRequired.value =
      (response.required_words || []).join(", ");
    extensionElements.dialogueResponseOptional.value =
      (response.optional_words || []).join(", ");
    extensionElements.dialogueResponseForbidden.value =
      (response.forbidden_words || []).join(", ");
    extensionElements.dialogueResponseSynonyms.value =
      JSON.stringify(response.synonyms || {}, null, 2);
    extensionElements.dialogueResponseCharacter.value =
      response.character_response;
    extensionElements.dialogueResponsePriority.value =
      response.priority || 100;
    extensionElements.dialogueResponseConditions.value =
      JSON.stringify(response.condition_rules || [], null, 2);
    extensionElements.dialogueResponseEffects.value =
      JSON.stringify(response.effect_rules || [], null, 2);
    extensionElements.dialogueResponseEnd.checked =
      response.end_conversation;
    extensionElements.dialogueResponseEnabled.checked =
      response.is_enabled !== false;

    populateDialogueNextNodeSelector();
    extensionElements.dialogueResponseNext.value =
      response.next_node_id || "";

    extensionOpen(extensionElements.dialogueResponseModal);
  }

  if (remove && confirm("Apagar esta Resposta?")) {
    const { error } = await state.client
      .from("dialogue_responses")
      .delete()
      .eq("id", remove.dataset.deleteDialogueResponse);

    if (error) throw error;

    await loadDialogueResponses(
      extensionState.selectedNodeId
    );
  }
}

async function saveDialogueResponse(event) {
  event.preventDefault();

  try {
    const id =
      extensionElements.dialogueResponseId.value || undefined;

    const payload = {
      id,
      node_id: extensionState.selectedNodeId,
      internal_name:
        extensionElements.dialogueResponseName.value.trim(),
      match_mode:
        extensionElements.dialogueResponseMode.value,
      exact_phrase:
        extensionElements.dialogueResponseExact.value.trim() || null,
      required_words:
        csvArray(extensionElements.dialogueResponseRequired.value),
      optional_words:
        csvArray(extensionElements.dialogueResponseOptional.value),
      forbidden_words:
        csvArray(extensionElements.dialogueResponseForbidden.value),
      synonyms:
        jsonValue(extensionElements.dialogueResponseSynonyms.value, {}),
      character_response:
        extensionElements.dialogueResponseCharacter.value.trim(),
      next_node_id:
        extensionElements.dialogueResponseNext.value || null,
      priority:
        Number(extensionElements.dialogueResponsePriority.value) || 100,
      condition_rules:
        jsonValue(
          extensionElements.dialogueResponseConditions.value,
          []
    ...
    extensionElements.dialogueResponseModal.classList.add(
      "is-hidden"
    );
    ...
    extensionMessage(
      extensionElements.dialogueResponseMessage,
      formatDatabaseError(error),
      true
    );
  }
...
function escapeExtensionHtml(value) {
  return String(value ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
