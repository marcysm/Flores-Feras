...
function cacheElements() {
  ...
  elements.scenarioModal =
    document.getElementById("scenario-modal");

  elements.scenarioModalPanel =
    elements.scenarioModal?.querySelector(".modal__panel") || null;

  elements.scenarioTitle =
    document.getElementById("scenario-title");

  elements.scenarioDescription =
    document.getElementById("scenario-description");

  elements.scenarioVisual =
    document.getElementById("scenario-visual");

  elements.scenarioDetails =
    document.getElementById("scenario-details");

  elements.dialogueShell =
    document.getElementById("dialogue-shell");

  elements.dialoguePortrait =
    document.getElementById("dialogue-portrait");

  elements.dialogueCharacterName =
    document.getElementById("dialogue-character-name");

  elements.dialogueMessages =
    document.getElementById("dialogue-messages");

  elements.endDialogueButton =
    document.getElementById("end-dialogue-button");

  elements.sceneCharacters =
    document.getElementById("scene-characters");

  elements.sceneCharacterList =
    document.getElementById("scene-character-list");
...
function configureInterfaceEvents() {
  ...
  elements.sceneCharacterList.addEventListener(
    "click",
    handleSceneCharacterClick
  );

  elements.endDialogueButton.addEventListener(
    "click",
    endActiveDialogue
  );
...
async function processInput(input) {
  /*
    Antes de enviar o texto ao motor narrativo normal,
    verificamos se ele é um comando espacial do Mapa.

    A função retorna handled: false quando o comando não
    representa movimentação. Nesse caso, o fluxo continua
    normalmente em process_game_input.
  */
  const {
    data: movementData,
    error: movementError
  } = await state.client.rpc(
    "process_map_movement_input",
    {
...
function applyRuntime(runtime) {
  ...
  refreshSceneExtensions().catch(error => {
    console.warn(
      "Não foi possível atualizar Cenário e Personagens:",
      error
    );
  });
}
...
async function handleCommandSubmit(event) {
  ...
  try {
    const extensionHandled =
      await processStoryExtensionCommand(originalInput);
...
function isScenarioCommand(value) {
...
function looksLikeDialogueStart(value) {
  const command = normalizeCommandText(value);

  return (
    command.startsWith("falar ") ||
    command.startsWith("falar com ") ||
    command.startsWith("conversar ") ||
    command.startsWith("conversar com ") ||
    command.startsWith("chamar ")
  );
}

async function processStoryExtensionCommand(input) {
  if (isScenarioCommand(input)) {
    await openCurrentScenario();
    return true;
  }

  if (state.dialogue.active) {
    await submitDialogueMessage(input);
    return true;
  }

  if (looksLikeDialogueStart(input)) {
    const started = await startDialogue(input);
...
async function refreshSceneExtensions() {
  if (!state.session?.id) {
    return;
  }
  ...
  state.dialogue.characters =
    normalizeArray(data.characters);

  renderSceneCharacters();

  if (data.active_dialogue) {
    state.dialogue.active = data.active_dialogue;

    if (state.dialogue.messages.length === 0) {
      appendDialogueMessage(
        "character",
        data.active_dialogue.speaker_text
      );
    }

    renderDialogueCharacter(data.active_dialogue);
    showDialogueShell();
  } else {
...
async function openCurrentScenario() {
  const { data, error } = await state.client.rpc(
    "get_scene_scenario",
    {
      p_session_id: state.session.id
  ...
  renderScenarioInterface(data);
}

function renderScenarioInterface(result = {}) {
  const scenario = result.scenario || {};
  const details = normalizeArray(result.details);
  ...
  elements.scenarioDescription.textContent =
    ...
    "Não há descrição registrada.";

  elements.scenarioVisual.replaceChildren();
  elements.scenarioDetails.replaceChildren();

  if (
    scenario.image_url &&
    isSafeMediaUrl(scenario.image_url)
  ) {
    ...
    elements.scenarioVisual.appendChild(image);
    elements.scenarioVisual.classList.remove("is-hidden");
  } else if (scenario.ascii_art) {
    const ascii = document.createElement("pre");
    ascii.textContent = scenario.ascii_art;

    elements.scenarioVisual.appendChild(ascii);
    elements.scenarioVisual.classList.remove("is-hidden");
  } else {
    elements.scenarioVisual.classList.add("is-hidden");
  }
  ...
  details.forEach(detail => {
    ...
    if (
      detail.image_url &&
      isSafeMediaUrl(detail.image_url)
    ) {
      const image = document.createElement("img");
      image.src = detail.image_url;
      image.alt =
        detail.image_alt_text ||
        detail.title ||
        "Detalhe do Cenário";
      card.appendChild(image);
    } else if (detail.ascii_art) {
      const ascii = document.createElement("pre");
      ascii.textContent = detail.ascii_art;
      card.appendChild(ascii);
    }

    elements.scenarioDetails.appendChild(card);
  });

  elements.scenarioDetails.classList.toggle(
    "is-hidden",
    details.length === 0
  );
...
function renderSceneCharacters() {
  elements.sceneCharacterList.replaceChildren();

  const characters = state.dialogue.characters;

  elements.sceneCharacters.classList.toggle(
    "is-hidden",
    characters.length === 0
  );
  ...
  characters.forEach(character => {
    ...
    button.type = "button";
    button.dataset.appearanceId = character.appearance_id;
    button.className = "scene-character-button";
    ...
    elements.sceneCharacterList.appendChild(button);
  });
}

async function handleSceneCharacterClick(event) {
  ...
  const character = state.dialogue.characters.find(
    entry =>
      entry.appearance_id ===
      button.dataset.appearanceId
  );
...
async function startDialogue(input) {
  const { data, error } = await state.client.rpc(
    "start_character_dialogue",
    {
      p_session_id: state.session.id,
      p_character_input: input
    }
  );
  ...
  renderDialogueCharacter(data.character || {});
...
async function submitDialogueMessage(input) {
  appendDialogueMessage("player", input);

  const { data, error } = await state.client.rpc(
    "process_character_dialogue",
    {
      p_session_id: state.session.id,
      p_input: input
    }
  ...
  if (!data?.success) {
    throw new Error(
      data?.error ||
      "Não foi possível processar a conversa."
    );
  }
  ...
  if (data.next_character_text) {
    appendDialogueMessage(
      "character",
      data.next_character_text
    );
  }
...
async function endActiveDialogue() {
  if (!state.dialogue.active || state.isProcessing) {
    return;
  }
...
function renderDialogueCharacter(character = {}) {
  elements.dialogueCharacterName.textContent =
    character.name || "PERSONAGEM";

  if (character.name_color) {
    elements.dialogueCharacterName.style.color =
      character.name_color;
  } else {
    elements.dialogueCharacterName.style.removeProperty(
      "color"
    );
  }

  elements.dialogueShell.dataset.side =
    character.portrait_side || "left";

  elements.dialoguePortrait.replaceChildren();

  if (
    character.portrait_url &&
    isSafeMediaUrl(character.portrait_url)
  ) {
    const image = document.createElement("img");
    image.src = character.portrait_url;
    image.alt =
      character.portrait_alt_text ||
      character.name ||
      "Retrato da personagem";
    elements.dialoguePortrait.appendChild(image);
  } else if (character.ascii_art) {
    const ascii = document.createElement("pre");
    ascii.textContent = character.ascii_art;
    elements.dialoguePortrait.appendChild(ascii);
  } else {
    const placeholder = document.createElement("span");
    placeholder.textContent =
      (character.name || "?").slice(0, 1).toUpperCase();
    elements.dialoguePortrait.appendChild(placeholder);
  }
}

function appendDialogueMessage(type, text) {
  if (!text) {
    return;
  }
  ...
  message.className =
    `dialogue-message dialogue-message--${type}`;
  ...
  elements.dialogueMessages.scrollTop =
    elements.dialogueMessages.scrollHeight;
}

function showDialogueShell() {
  elements.dialogueShell.classList.remove("is-hidden");
  elements.sceneCharacters.classList.add("is-hidden");
}

function hideDialogueShell() {
  elements.dialogueShell.classList.add("is-hidden");
  elements.dialogueMessages.replaceChildren();

  if (state.dialogue.characters.length > 0) {
    elements.sceneCharacters.classList.remove("is-hidden");
  }
}
